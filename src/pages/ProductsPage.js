...
import { getFacultad } from '../redux/slice/apiSlice';
...
export default function ProductsPage() {
  ...
  return (
    <>
      <Helmet>
        <title> Docentes | Escalafón </title>
      </Helmet>
          ...
          {isLoading ?  <div style={{height:'100%',width:'100', display:'flex',justifyContent:'center', alignItems:'center'}}><CircularProgress /></div>:
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                ...
                <TableBody>
                  {facultades.map((row) => {
                    const { codigo, facultad } = row;
                    const selectedUser = selected.indexOf(codigo) !== -1;

                    return (
                      <TableRow hover key={codigo} tabIndex={-1} role="checkbox" selected={selectedUser}>
